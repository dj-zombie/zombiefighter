  ...
  for (int j = 0; j < N_MUX; j++) {
    for (int i = 0; i < N_BUTTONS_PER_MUX[j]; i++) {
      ...
      if (buttonCState[i + nButtonsPerMuxSum] > buttonMuxThreshold) {
        ...
        if (!lightShowOn) {
          leds[i + nButtonsPerMuxSum] = CRGB::Black; FastLED.show();
        }
        ...
        //leds[i + nButtonsPerMuxSum] = CRGB::Green; FastLED.show();
        leds[i + nButtonsPerMuxSum] = CHSV( random8(), 255, 255);

        if (buttonCState[i + nButtonsPerMuxSum] = LOW) {
          lightShowOn = false;
        }
      }
    }
